...
# 用法: ./sweep.sh [模拟器UDID] [delta] [luma|chroma]
set -euo pipefail
...
echo "plane=$PLANE  delta=${DELTA:-默认}  模拟器=$UDID"
for PAGE in plain white text photo dark mixed; do
  xcrun simctl terminate "$UDID" "$BUNDLE" 2>/dev/null || true
  if [ -n "$DELTA" ]; then
    SIMCTL_CHILD_BW_PAGE=$PAGE SIMCTL_CHILD_BW_PLANE=$PLANE SIMCTL_CHILD_BW_DELTA=$DELTA \
      xcrun simctl launch "$UDID" "$BUNDLE" >/dev/null
  else
    SIMCTL_CHILD_BW_PAGE=$PAGE SIMCTL_CHILD_BW_PLANE=$PLANE \
      xcrun simctl launch "$UDID" "$BUNDLE" >/dev/null
  fi
  sleep 4
  xcrun simctl io "$UDID" screenshot "/tmp/bw_${PLANE}_$PAGE.png" >/dev/null 2>&1
  printf "%-6s " "$PAGE"
  "$ROOT/.build/release/bwdecode" "/tmp/bw_${PLANE}_$PAGE.png" --plane "$PLANE" | tail -1
done
