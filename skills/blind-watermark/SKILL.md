...
静默裁决。已知比例可传 `--scale 0.837`；比例未知时用 `--auto`：解码端会先用**比例尺**（水平自相关读 block 周期）
粗定位 scale，stderr 会打一行「比例尺粗定位：chroma scale≈x.xxx（置信 y.yy）」，置信不足或快路径失败
自动退回完整网格。v4 没有 scale 搜索 —— 被 IM 转发缩放的截图只能用 v5.2 解。自动路径在 0.50...1.50
粗网格上继续局部精搜，并使用
